  ...
  context 'with single target' do
      ...
      it 'runs successfully' do
        ...
        %w[foo-123 baz-123].each do |snapshot|
          expect_task('aptly::snapshot_drop').
            with_targets(plan_targets).
            with_params('name' => snapshot, '_run_as' => 'aptly').
    ...
    context 'with pattern' do
      it 'runs successfully' do
        ...
        # Drop 'foo-123' (matches pattern, not in publish_list)
        expect_task('aptly::snapshot_drop').
          with_targets(plan_targets).
          with_params('name' => 'foo-123', '_run_as' => 'aptly').
          always_return({ 'snapshot_drop' => true })

        result = run_plan(plan, 'targets' => plan_targets, 'pattern' => pattern)
        expect(result.ok?).to be(true)
      end
    end
